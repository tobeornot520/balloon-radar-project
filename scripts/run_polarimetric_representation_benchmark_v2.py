...
def main() -> None:
    ...
    if args.gate_gamma <= 0:
        raise ValueError("gate gamma must be positive")
    ...
    plan = {
        ...
        "scientific_role": (
            "sample-independent comparison of Power2, RI4 and power-confidence-gated "
            "explicit H/V representations; no scan context"
        ),
    }
    ...
    for fold in args.folds:
        ...
        for mode in args.modes:
            name = f"polar_repr_v2_{mode}_v4_fold{fold:02d}_seed42_{scope}"
            root = PROJECT_ROOT / "results/experiments" / name
            if complete(name):
            ...
            command = [
                ...
                "--gate-low-percentile", str(args.gate_low_percentile),
                "--gate-high-percentile", str(args.gate_high_percentile),
                "--gate-gamma", str(args.gate_gamma),
                "--seed", "42",
            ]
            if debug_per_class:
