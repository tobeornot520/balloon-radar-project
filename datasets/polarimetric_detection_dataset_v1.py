...
VALID_INPUT_MODES = {"power2", "ri4", "polar6", "ri8"}
NUM_PULSES = 128
NUM_GATES = 100
...
def representation_channels(mode: str) -> tuple[str, ...]:
    mapping = {
        ...
        "polar6": (
            "H_power", "V_power", "relative_ZDR_like", "local_rho_HV",
            "cos_relative_phase", "sin_relative_phase",
        ),
        "ri8": (
            "Re_H", "Im_H", "Re_V", "Im_V", "relative_ZDR_like",
            "local_rho_HV", "cos_relative_phase", "sin_relative_phase",
        ),
    }
...
class PolarimetricDetectionDatasetV1(Dataset):
    """Manifest-based, sample-independent H/V representation benchmark dataset.
    ...
    def __init__(
        ...
        input_mode: Literal["power2", "ri4", "polar6", "ri8"] = "power2",
    ...
    def _make_input(self, h: np.ndarray, v: np.ndarray) -> np.ndarray:
        ...
        if self.input_mode == "power2":
            ...
            array = make_polar6(features, zdr_clip_db=self.polar_config.zdr_clip_db)
        else:
            array = make_ri8(features, zdr_clip_db=self.polar_config.zdr_clip_db)
