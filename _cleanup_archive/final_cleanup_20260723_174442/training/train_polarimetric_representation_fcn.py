...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train a fixed-capacity sample-independent polarimetric representation FCN."
    )
    parser.add_argument("--name", required=True)
    parser.add_argument("--manifest-path", required=True)
    parser.add_argument(
        "--input-mode", choices=["power2", "ri4", "polar6", "ri8"], required=True
    )
...
def validate_args(args: argparse.Namespace) -> None:
    positive = (
        ...
        "velocity_window", "range_window", "zdr_clip_db",
...
def main() -> None:
    ...
    base_datasets = {
        split: PolarimetricDetectionDatasetV1(
    ...
    summary = {
        ...
        "benchmark": "sample_independent_polarimetric_representation_v1",
        "input_mode": args.input_mode,
        "active_channels": list(representation_channels(args.input_mode)),
        "canonical_channels": 8,
