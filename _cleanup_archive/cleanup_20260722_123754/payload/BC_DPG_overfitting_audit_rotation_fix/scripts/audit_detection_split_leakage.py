...
def group_role_table(
    ...
    for key_value, group in all_rows.groupby(
        ...
        # Count fold assignments, not sample rows. A scan group can contain
        # many beams/azimuths, so counting rows falsely reports rotation
        # anomalies even when the whole group moves atomically by fold.
        fold_role_counts = Counter(role_by_fold.values())
        mixed_role_fold_count = sum(
            1 for role in role_by_fold.values() if "," in role
        )
        ...
        row = {
            ...
            "train_count": int(fold_role_counts.get("train", 0)),
            "val_count": int(fold_role_counts.get("val", 0)),
            "test_count": int(fold_role_counts.get("test", 0)),
            "mixed_role_fold_count": int(mixed_role_fold_count),
...
def add_rotation_flags(
    ...
    result["rotation_ok"] = (
        complete_presence
        & (result["mixed_role_fold_count"] == 0)
