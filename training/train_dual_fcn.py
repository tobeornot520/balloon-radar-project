...
def main():
    ...
    first_conv_key = (
        "network.0.block.0.weight"
    )

    if first_conv_key not in pretrained_state:
        raise KeyError(
            "预训练权重缺少首层卷积："
            f"{first_conv_key}"
        )

    pretrained_input_channels = int(
        pretrained_state[
            first_conv_key
        ].shape[1]
    )

    if pretrained_input_channels != input_channels:
        raise ValueError(
            "预训练模型输入通道与当前实验不一致："
            f"预训练={pretrained_input_channels}，"
            f"当前={input_channels}。"
            "请先为当前channel训练对应Sigma模型。"
        )
