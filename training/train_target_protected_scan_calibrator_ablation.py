...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default="full",
        help="BC-DPG-FCN v3 ablation switch.",
    )
...
def apply_ablation_mode(
    ...
    """Convert one named ablation into explicit, auditable switches.

    The model architecture is kept unchanged. ``no_scan_context`` replaces
    all 12 scan-group features with zeros, so parameter count and optimizer
    settings remain identical to the full model.
    """
    args.zero_group_features = False

    if args.ablation_mode == "full":
        return args
    if args.ablation_mode == "no_scan_context":
        args.zero_group_features = True
    elif args.ablation_mode == "no_background_classification":
        args.background_classification_weight = 0.0
    elif args.ablation_mode == "no_background_tail":
        args.background_tail_weight = 0.0
    elif args.ablation_mode == "no_target_keep":
        args.target_keep_weight = 0.0
    elif args.ablation_mode == "no_pairwise":
        args.pairwise_weight = 0.0
    elif args.ablation_mode == "no_shift_selectivity":
        args.shift_selectivity_weight = 0.0
    elif args.ablation_mode == "no_shift_regularization":
        args.shift_regularization = 0.0
    elif args.ablation_mode == "no_target_protection":
        args.target_keep_weight = 0.0
        args.pairwise_weight = 0.0
        args.shift_selectivity_weight = 0.0
    else:
        raise ValueError(
            f"Unsupported ablation mode: {args.ablation_mode}"
        )
    return args
...
def precompute_split(
    ...
    if bool(getattr(args, "zero_group_features", False)):
        group_features = np.zeros_like(
            group_features,
            dtype=np.float32,
        )
    return (
...
def main() -> None:
    args = apply_ablation_mode(parse_args())
    set_seed(args.seed)
    ...
    print(f"Zero scan context      : {args.zero_group_features}")
