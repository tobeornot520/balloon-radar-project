...
- 当前方向完成门为 `3/6`，状态是 `BLOCKED_EXTERNAL`。
...
1. 为什么当前项目还不是空飘球分类项目的完成版本？
2. 为什么 `56/830` 不能写成严格实时部署性能？
3. 为什么 LAT-MRICD 不能解决当前 H/V 极化问题？
4. 为什么随机拆分 batch、scan 或 session 会造成虚高结果？
5. 自己准备认领什么任务，交付物和停止条件是什么？
...
| D04-P0 | P0 | 工作台已就绪 | 雷达/数据分析 | 11 例 P0 复核 CSV 和审计摘要 |
| D01-D02 | P0 | 等待外部事实 | 硬件/沟通 | H/V、IQ、PRF、坐标的可归因答复或 unknown 结论 |
| D10 | P0 | 等待学长材料 | 模型复现 | Tian 单样本输入-标签-输出对齐或条件缺口表 |
...
| 角色 | 主任务 | 维护任务 |
|---|---|---|
| A：雷达与数据 | D04-P0、D01-D02、外场记录 | 数据卡和物理解释审核 |
| B：特征与模型 | D17-NX、D17-HX、后续极化/时域 | 特征 schema 和负结果 |
| C：复现与工程 | D10、D15、测试和分享包 | Git、哈希和周报检查 |
...
| 第 3 天 | 环境/测试验收、提交任务认领表 | D04 工作台或 LAT 审计复跑 |
