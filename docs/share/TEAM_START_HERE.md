...
版本：2026-08-07 V9（随唯一 V15 分享包发布）

适用对象：此前没有参与过项目开发的新组员，以及准备继续参与下一阶段的现有成员

负责人：项目负责人指定

执行顺序：先完成 ONB-01，再完成最小试做；角色和权限确认后，才认领正式任务。

## 1. 完成本手册后应达到什么状态

本手册把入组、试做和正式分工分开。完成 ONB-01 时应能独立做到：
...
5. 完成唯一 V15 分享包校验；具备源码权限时如实记录环境和测试结果。

ONB-01 通过后，再完成一项 3-6 小时最小试做并由 reviewer 复核。角色和权限确认后，正式
任务才填写任务认领表；周报只用于已经启动的正式任务。未完成这些阶段时，不分配正式训练、
阈值选择或论文数字整理工作。
...
## 4. 72 小时 ONB-01 基础入组流程

前 48 小时完成校验和最低阅读，72 小时内完成五问与 ONB-01 判定；通过后才进入第 7 天前
完成的最小试做和角色确认。
...
5. 当前还有什么能做、什么不能做；自己对哪类最小试做感兴趣？兴趣只用于匹配，不构成正式认领。
...
成员只提交个人入组清单和五问回答/评审记录，不提交正式任务认领表。判定为 `pass`、
`needs_followup` 或 `rejected`；`pass` 表示完成项目内部基础入组或继续参与确认。学校正式
名单、署名和学分仍按指导老师及学校规则处理。
...
## 6. 正式任务认领方法（不属于 ONB-01）

只有完成 ONB-01、最小试做以及角色和权限确认后，才使用
[正式任务认领表](assets/templates/team_task_claim_template_v2.csv)。每个正式任务必须具备：

ONB-01 不使用本表，提交格式见
[ONB-01 基础入组确认说明](docs/ONB_01_SUBMISSION_GUIDE_ZH.md)。最小试做使用
...
当前没有可由新人立即启动的正式模型实验。以下三类入口必须区分：

| 阶段 | ID | 性质 | 交付/动作 |
|---|---|---|---|
| 基础入组 | ONB-01 | 不是研究任务 | 个人入组清单、五问回答/评审记录；不提交正式任务认领表 |
| 最小试做 | Q1-Q6 | 入组后的 3-6 小时角色评估 | 使用 trial 模板，交付实际文件、命令或审查记录 |
| 正式任务 | D/PUB/FIELD | 长期或持续项目分工 | 角色和权限确认后，使用正式任务认领表申请 |

Q1-Q6 的完整定义见 `docs/19_TEAM_QUALIFICATION_AND_ROLE_SCREENING_ZH.md` 第 5 节。下面的
D/PUB 项用于说明冻结证据、阻塞条件和未来正式任务范围，不表示都可立即认领。
...
| D16 | P1 | 持续 | 文档/工程 | Git 清洁、包内敏感信息审计、唯一当前分享包 |
...
本节只适用于角色与权限已经确认、正式认领已经批准的任务；ONB-01 和 Q1-Q6 最小试做分别按
对应指南与 trial 模板执行。
...
正式任务启动后使用 [统一周报模板](assets/templates/TEAM_WEEKLY_REPORT_TEMPLATE_ZH.md)。ONB-01
和一次性最小试做不要求提交周报。周报每周只回答八件事：
...
以下只是角色和权限评估完成后的分工参考，不是 ONB-01 的选择题，也不代表表中任务当前都已
开放。正式启动仍须负责人逐项确认任务状态、输入和门禁。
...
| 第 2-3 天 | 五问、个人清单和 ONB-01 判定 | 有仓库权限者如实记录环境/测试结果 |
| 第 3 天后 | 只给 ONB-01 已通过者分配一项 Q1-Q6 最小试做 | 填写 trial 模板，不接触未授权数据 |
| 第 4-6 天 | 完成 3-6 小时试做，保留命令、产物和失败记录 | reviewer 只按预先公布的评分标准复核 |
| 第 7 天 | 确认初始角色和最小必要权限 | 合适且当前可启动时，再讨论第一项正式任务 |

第一周不以“训练出一个准确率”为目标。ONB-01 的完成标准是基础认知与证据边界通过；最小
试做的完成标准是过程真实、产物可复核并形成角色判断。正式任务卡不属于基础入组提交物。

## 16. 正式任务完成定义

以下定义只适用于已经批准的正式任务，不用于倒推 ONB-01 或最小试做是否合格。一个正式任务
只有同时满足以下条件才算完成：
...
- 主持五问并记录 ONB-01 的独立判定；
- 只为 ONB-01 已通过者安排 3-6 小时最小试做，指定 reviewer；
- 根据试做证据确认角色和最小必要权限，再决定是否批准正式任务认领；
...
- `assets/templates/team_onboarding_checklist_template_v2.csv`：ONB-01 基础入组记录，不含任务认领；
- `assets/templates/team_trial_task_template_v1.csv`：Q1-Q6 最小试做安排与复核记录；
- `assets/templates/team_qualification_scorecard_template_v2.csv`：试做后的评分、补验、角色和权限记录；
- `assets/templates/team_task_claim_template_v2.csv`：角色确认后的正式任务认领与门禁；
