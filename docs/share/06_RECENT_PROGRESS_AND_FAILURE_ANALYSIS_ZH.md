...
外场准备新增 H/V IQ 内容探针。它可以实际打开 MATLAB v5/v7.3 文件，检查变量、复数性、
有限值、I/Q 变化、H/V 同形和设备期望形状；逐文件路径与哈希保持本地。当前包没有真实
外场数据，默认合同也故意不填期望形状，因此 `real_data_audit_completed=false`。探针不能
证明 H/V 相干、标定、PRF 或物理轴，五道外场门仍保持阻塞。
