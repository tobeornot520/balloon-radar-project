...
| D04 | 186/187 个历史误警具体是什么物理类型 | P0 人工复核已完成；完整物理类型仍未知 | 11 例导出 CSV 已通过审计：9 例近零多普勒峰、2 例宽结构、11 例物理类别 unknown | 我维护聚合结论；未来由现场记录补物理标签 | P0 的 11 例可见结构、备注和证据来源完整；无证据项保持 unknown | 保留当前聚合结论；新同条件现场记录到位后再扩展物理类别复核 |
...
| D16 | 分享包和 Git 是否保持可交付 | 已完成（V10） | 只保留 V10 和 V9；原始聊天/数据/权重留本地并忽略；新文档和构建脚本提交本地 Git | 我维护，你确认分享边界 | 包内无敏感路径、原始数据、权重和聊天记录；V10 manifest 源提交等于构建时 HEAD；完成门逐项核验证据非空 | 发布新版本时再按“最新+上一版”规则滚动维护 |
