...
| D16 | 分享包和 Git 是否保持可交付 | 已完成（V12） | 只保留 V12 和 V11；原始聊天/数据/权重/逐样本虚警与目标库留本地并忽略；新文档和构建脚本提交本地 Git | 我维护，你确认分享边界 | 包内无敏感路径、原始数据、权重、聊天记录、sample ID 或来源标识；V12 manifest 源提交等于构建时 HEAD；完成门逐项核验证据非空 | 发布新版本时再按“最新+上一版”规则滚动维护 |
