...
先阅读 `docs/LAT_MRICD_GROUPED_BASELINE_PROTOCOL_V1.md`。V11 分享包只支持 A 级证据复核；
...
V11 分享包可直接做 A 级复核：阅读 `evidence/26_LSS_DAUR_READ_ONLY_AUDIT.md`、核对
...
不修改原始文件，也不训练模型。V11 仅纳入报告、summary 与 39/6/3 行三张聚合表，不纳入
...
V11 分享包可直接复核 `evidence/27_LSS_HSR_L_V2_READ_ONLY_AUDIT.md`、对应 JSON，以及
...
V11 包中的 `evidence/28_LSS_FMCWR_2_V1_READ_ONLY_AUDIT.md`、JSON、6 行 archive
